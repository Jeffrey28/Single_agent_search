...
We let the sensor share the same state as the robot (i.e. position and heading angle).
...
In practice, the receiving of a measurement corresponds to a finite covariance $R\succeq 0$ and the absence of measurement corresponds to the limiting case of $\sigma\rightarrow\infty$.
...
\end{equation}
to be an indicator function that reflects whether the target is within sensing domain (a measurement is thus received) and the case that the target is outside of sensing domain (therefore the measurement is missing).
