...
Despite the popularity of statistics dissemination strategy, exchanging statistics can consume high communication resources, which can be infeasible in vast area or complex environment.
...
\begin{rem}
	...
	On the contrary, the length of transmitted data for a statistics dissemination approach is $O(M^2)$, which is in the order of environment size. 
...
% \lim\limits_{k\rightarrow \infty}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})}\overset{P}{\longrightarrow}\sum\limits_{j=1}^{N}p^*_j\ln \frac{p_j}{p^*_j}+(1-p^*_j)\ln \frac{1-p_j}{1-p^*_j}
 \end{equation}\normalsize
...
They would also like to thank Yuting Wei in Department of Statistics, UC Berkeley for her sincere help and fruitful discussion on the consistency proof.
