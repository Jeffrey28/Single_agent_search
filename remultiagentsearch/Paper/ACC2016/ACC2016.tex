...
Distributed filtering that focuses on using a group of networked robots to collectively infer environment status has been used for various applications, such as intruder detection \cite{chamberland2007wireless}, target tracking \todohere{change reference, more application-oriented} \cite{beaudeau2012target} and micro-environmental monitoring \cite{cao2008development}. 
...
Gu (2007) presented a distributed particle filter for target tracking over sensor networks \cite{gu2007distributed}. 
...
As a generic method for nonlinear system and arbitrary noise distribution, distributed Bayesian filter (DBF) has received increasing interest during past years \cite{keylist}, which is the focus of this study.
...
\todohere{check if FC method requires the all info before processing}
\todohere{need to give one more level of details for FC. refer to Hlinka's paper.}
...
\todohere{NB method is also verbose and not detailed.}
...
Thus far, most studies on NB-based DBF have mainly focused on the so-called \textit{statistics dissemination} strategy that each robot actually exchanges posterior distributions or likelihood functions to neighboring robots for distributed estimation.
...
This paper proposes a local-exchange-of-observation (LIFO) strategy for distributed Bayesian filters (DBF) for undirected and connected communication topology. 
...
The proposed LIFO-DBF has following properties:
...
In this study, formal proofs of the consistency of LIFO-DBF and consensus of robots' target PDFs are provided.

The rest of this paper is organized as follows: 
The distributed SAT of target is formulated in \cref{sec:prob_form}.
The LIFO-DBF algorithm is described in \cref{sec:LIFO-dbf}, followed by the proof of consistency and consensus in \cref{sec:consist_proof}.
Simulation results of LIFO-DBF and conclusions are presented in \cref{sec:sim} and \cref{sec:conclu}, respectively.
...
\begin{rem}
	...
	To the best knowledge of authors, there is no proof of consistency of individual PDFs \todohere{add reference}.
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\caption{(a)-(d): The $1^\text{st}$ robot's individual PDFs at different time; (e) All robots' individual PDFs at time $30$.\todohere{revise symbols} $\square$ denotes the current robot and $\star$ represent other robots. + stands for the target.}
