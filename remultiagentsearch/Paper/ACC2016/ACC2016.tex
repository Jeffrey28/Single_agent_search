...
\title{\LARGE \bf
Distributed Environmental Estiamtion Using A Group of UGVs Under Dynamically Changing Interaction Topologies}
...
This paper presents a measurement dissemination-based distributed Bayesian filtering (DBF) method for a network of multiple unmanned ground vehicles (UGVs), utilizing the Latest-In-and-Full-Out (LIFO) local exchange protocol of observations.
...
For the static target, each UGV locally fuses the newly received observations while for the moving target, a set of historical observations is stored and updated. 
%Upon obtaining the latest available observations of all UGVs, an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps. 
...
The design of distributed filtering algorithms is closely related to communication topology of multi-UGV network, which can be classified into two types: fusion center (FC)-based and neighborhood (NB)-based.
...
%At each filtering cycle, fusion center calculates the global state estimate only after receiving latest local estimates of all UGVs \cite{zuo2006bandwidth,vemula2006target}.
% has been a common structure for distributed filtering, in which local information collected by UGVs is transmitted (possibly via multi-hopping) to the fusion center for forming global estimation \cite{zuo2006bandwidth,ribeiro2006bandwidth}. 
%FC-based DBF is efficient for estimation in that it can collectively utilize all UGVs' information and thus useful for applications that only require information at a single central unit, such as in environmental monitoring.
%However, FC-based DBF requires constant communication link between each UGV and the center, which is challenging for applications of UGVs in vast or complex areas.
...
%Besides, filtering is locally conducted on each UGV, which requires less computation power compared to that in the fusion center.

%instead of communicating with a fusion center, each UGV only exchanges information with neighboring UGVs and forms local estimation of the environment state. 
%NB-based DBF is advantageous over FC-based DBF in that no central unit is required, thus suitable for applications in which maintaining communication link between UGVs and center is challenging, such as in disaster situations.
%Besides, state estimation is locally conducted on each UGV, which requires less computation power compared to that in the fusion center.
...
\section{Latest-In-and-Full-Out Protocol for Dynamically Changing Topologies}\label{sec:prob_form}
Consider a network of $N$ UGVs in a bounded two-dimensional space $S$. 
Each UGV is equipped with a binary sensor for environmental perception. Due to the limit of communication range, each UGV can only exchange observations with its neighbors. 
...
\end{equation}\normalsize
where $x^T_k$ denotes the target position; $x^{R,i}_k$ is the UGV position.
...
Consider a simple\footnote{An undirected graph $G$ is \textit{simple} if it has no self-loops or repeated edges, i.e., $\left( i,j\right)\in E,\,only if i\neq j$ and $E$ only contains distinct elements. A graph is \textit{connected} when there is a path between every pair of vertices.}, undirected graph $G=(V,E)$ to represent the communication topology of the UGV network, where $V=\left\lbrace 1,\dots,N\right\rbrace $ represents the index set of UGVs and $E=V\times V$ denotes the edge set. 
The \textit{adjacency matrix} $M=\left[ m_{ij}\right] $ of graph $G$ describes the communication topology:
...
The communication topology can be dynamically changing due to limited communication range, varying team formation or link failure.
Let $\bar{G}=\left\lbrace G_1,G_2,\dots,G_L\right\rbrace $ denote the set of all possible undirected communication graphs defined for the network of UGV.
The adjacency matrix associated with graph $G_l,\,l=\left\lbrace 1,2,\dots,L \right\rbrace $ is denoted as $M^l=[m^l_{ij}]$.
It is easy to know that $\bar{G}$ has finite elements.
Define the \textit{union} of a collection of simple graphs $\left\lbrace G_{i_1},G_{i_2},\dots,G_{i_k} \right\rbrace \subset \bar{G}$ as the undirected graph with nodes in $V$ and edge set given by the union of edge sets of $G_{i_j},\,j=1\dots,k$.
...
The \textit{direct neighborhood} of $i^\text{th}$ UGV under topology $G_{l}$ is defined as $\mathcal{N}_i(G_{l})=\left\lbrace j|m^l_{ij}=1,\forall j\in\left\lbrace1,\dots,N \right\rbrace \right\rbrace $. 
All the UGVs in $\mathcal{N}_i(G_{l})$ can directly exchange information with $i^\text{th}$ UGV. 
In addition to direct neighborhood, another set called \textit{available neighborhood} is defined as $\mathcal{Q}_i(G_{l})$, which contains indices of UGVs whose observations can be received by the $i^\text{th}$ UGV given a specific observation exchange protocol and the topology $G_{l}$. 
...
All of them are simple and undirected graphs.
...
For explanation of DBF in \cref{sec:LIFO-dbf}, we define a \textit{new observation set} $\mathbf{z}^{new,i}_k$ for each UGV to denote the set of observations that the $i^\text{th}$ UGV receives and stores in its CB at $k$.
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		Add $z^i_k$ to $\mathbf{z}^{new,i}_k$.
		...
		\text{If}\quad l_\text{latest} &> z^j_{(k-1)^i_j},\quad\text{add $z^i_{(k-1)^{l_\text{latest}}_j}$ \text{to} $\mathbf{z}^{new,i}_k$.}\\
...
\begin{rem}
	...
	To be specific, consider an $D\times D$ grid environment with a network of $N$ UGVs, the transmitted data of LIFO between each pair of UGVs are only the CB of each UGV and the corresponding UGV positions where observations were made, the length of which is $O(N)$. 
...
Consider an undirected network of $N$ UGVs.
Let $G[k]\in\bar{G} $ be the communication topology at time $t=kT$. 
If there exists an infinite sequence of contiguous, nonempty and uniformly bounded time intervals $\left[k_m,k_{m+1} \right),\,m=1,2,\dots$, starting at $k_1=0$, with the property that the union of graphs across each such interval is jointly connected, then any pair of UGVs can exchange observations under LIFO. In addition, the delay between an arbitrary pair of UGVs is no greater than $(N-1)T_u$, where $T_u=\sup\limits_{m=1,2,\dots}\left( k_{m+1}-k_m\right) T$ is the upper bound of intervals.
\end{prop}
...
Consider the data transmission between $i^\text{th}$ and $j^\text{th}$ UGV.
%Without loss of generality, we focus on the transmission of $z^i_0$, the observation at time $0$ by $i^\text{th}$ UGV.
Since the union of graphs across time interval $\left[k_1,k_2 \right)$ is jointly connected, $i^\text{th}$ UGV is connected to at least one another UGV at some time step, i.e., $\exists l_1\in V,\,t_1\in \left[k_1,k_2 \right)$ such that $i\in\mathcal{N}_{l_1}(G_{t_1})$.
This implies that observation $z^i_{t_1}$ is received and stored in the CB of $l_1^\text{th}$ UGV at $t_1+1$ under LIFO.
Therefore, we have shown that at least one UGV have received observation from $i^\text{th}$ UGV by $k_2$.
...
By using similar reasoning as before, it is easy to understand that $\exists l_2\in V,\,t_2\in \left[k_2,k_3 \right)$ such that $i\in\mathcal{N}_{l_2}(G_{t_2})$ or $l_1\in\mathcal{N}_{l_2}(G_{t_2})$.
...
This occurs if $l_2^\text{th}$ UGV has received a newer observation $z^i_{t^\prime_2},\,t^\prime_2>t_1$ from UGVs other than $l_1$.
In both cases, it is shown that at least two UGVs have received and stored an observation from $i^\text{th}$ UGV by $k_3$.
Using similar derivation, it can be shown that $N-1$ UGVs, except $i^\text{th}$ UGV, will receive and store an observation from $i$ no later by $k_{N}$.
Therefore, the transmission delay between an arbitrary pair of UGVs is no greater than $(N-1)T_u$.
\end{proof}
%\vspace{\baselineskip}
\medskip
\begin{cor}\label{cor1}
For the same network condition in \Cref{prop1}, all elements in $\mathbf{z}^{CB,i}_k$ under LIFO become filled when $k\geq (N-1)T_u$, i.e., $\mathcal{Q}_i = \left\lbrace1,\dots,N \right\rbrace \setminus \left\lbrace i\right\rbrace $.	
Additionally, each element keeps getting updated after finite intervals.
\end{cor}
\begin{proof}
%In a network of $N$ UGVs, the maximal length of shortest paths is no greater than $N-1$. 
Based on \Cref{prop1}, the transmission delay between an arbitrary pair of UGVs is no greater than $(N-1)T_u$.
Therefore, CBs of all UGV will be filled when $k\geq (N-1)T_u$.
In addition, each element in CBs gets updated after finite interval no greater than $(N-1)T_u$.
...
$P^i_{pdf}(x^T_{k}|\mathbf{z}^{new,i}_{1:k})$, where $\mathbf{z}^{new,i}_{1:k}$ denotes the set of new observation set by $i^\text{th}$ UGV from time $1$ to $k$.
...
The individual PDF is initialized as $P^i_{pdf}(x^T_0|\mathbf{z}^{new,i}_0)=P(x^T_0)$,
%$P^i_{pdf}(x^T_0|z^i_0,z^{\mathcal{Q}_i}_0)=P(x^T_0)$, 
given all available prior information including past experience and environmental knowledge. Under the framework of DBF, the individual PDF is recursively estimated by two steps, i.e., prediction step and updating step, based on observations of $i^\text{th}$ UGV and UGVs in $\mathcal{Q}_i$.
...
The $i^\text{th}$ individual PDF is then updated by Bayes' formula using the set of newly received observations at time $k$, $\mathbf{z}^{new,i}_k$:
...
\end{align*}\normalsize
and $P^i_{pdf}(x^T_k|\mathbf{z}^{new,i}_{1:k})$ is called posterior individual PDF; $P(z^j_{k^i_j}|x^T_k)$ is the likelihood function of observation $z^j_{k^i_j}$, described in \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0}.
...
This section proves consistency and consensus of LIFO-DBF. 
Considering $S$ is finite and $x^{T^*}$ is the true location of target, the consistency of LIFO-DBF for static UGVs is stated as follows: 
...
\begin{proof}	
For the purpose of clarity, define a time set $\mathscr{K}^{i}_j,\,j\in\left\lbrace 1,\dots,N\right\rbrace $ for $i^\text{th}$ UGV that contains time steps when associated observations by $j^\text{th}$ UGV are in $\mathbf{z}^{new,i}_{1:k}$.
...
%\end{subequations}\normalsize
\small\begin{equation}
P^i_{pdf}(x^T|\mathbf{z}^{new,i}_{1:k})=\frac{P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l\in\mathscr{K}^{i}_j}P(z^j_l|x^T)}{\sum\limits_{x^T\in S}P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l\in\mathscr{K}^{i}_j}P(z^j_l|x^T)},
\end{equation}\normalsize

where $P^i_{pdf}$ is $i^\text{th}$ initial individual PDF. 
It is known from \Cref{cor1} that the cardinality of $\mathscr{K}^i_j$ has following property: $k-(N-1)T_u<|\mathscr{K}^i_j|\leq k$.
...
\frac{P^i_{pdf}(x^T|\mathbf{z}^{new,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{new,i}_{1:k})}=\frac{P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l\in\mathscr{K}^{i}_j}P(z^j_l|x^T)}{P^i_{pdf}(x^{T^*})\prod\limits_{j=1}^{N}\prod\limits_{l\in\mathscr{K}^{i}_j}P(z^j_l|x^{T^*})}
\end{equation}\normalsize

Take the logarithm of \Cref{eqn:cmp} and average it over $k$ steps:
\small\begin{equation}\label{eqn:cmp_log}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^{new,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{new,i}_{1:k})}=\frac{1}{k}\ln\frac{P^i_{pdf}(x^T)}{P^i_{pdf}(x^{T^*})}+\sum\limits_{j=1}^{N}\frac{1}{k}\sum\limits_{l\in\mathscr{K}^{i}_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}.
\end{equation}\normalsize
...
Utilizing the facts: (1) $z^j_l$ are conditionally independent samples from $B(1,p_j^*)$ and (2) $k-(N-1)T_u<|\mathscr{K}^i_j|\leq k$, the law of large numbers yields
\small\begin{equation*}
%\lim\limits_{k\rightarrow \infty}
\frac{1}{k}\sum\limits_{l\in\mathscr{K}^i_j}z^j_l\overset{P}{\longrightarrow}p^*_j,\quad 
%\lim\limits_{k\rightarrow\infty}
\frac{1}{k}(|\mathscr{K}^i_j|-\sum\limits_{l\in\mathscr{K}^i_j}z^j_l)\overset{P}{\longrightarrow}1-p^*_j
\end{equation*}\normalsize
...
%\lim\limits_{k\rightarrow \infty}
\frac{1}{k}\sum\limits_{l\in\mathscr{K}^i_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}\overset{P}{\longrightarrow}p^*_j\ln \frac{p_j}{p^*_j}+(1-p^*_j)\ln \frac{1-p_j}{1-p^*_j}
\end{equation}\normalsize
...
%Here, the main idea is to classify UGV observation positions into two disjoint sets: \textit{infinite-observation spots} that contains positions where a UGV makes infinite observations, and \textit{finite-observation spots} that contains positions where the UGV makes finite observations.
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\caption{(a)-(d) The $1^\text{st}$ UGV's individual PDFs at different times; (e) All UGVs' individual PDFs at time $30$. The square denotes the current UGV and stars represent other UGVs. The cross stands for the target.}
...
\begin{figure}%[thpb]
	...
	\end{subfigure}	
	\begin{subfigure}[b]{0.19\textwidth}
		\includegraphics[width=\textwidth]{figures/mov_sen_mov_tar_single_40}
		\caption{Step 40}\label{fig:mov_sen_mov_tar_sing4}
	\end{subfigure}
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\textwidth]{figures/mov_sen_mov_tar_40}
		\caption{Step 40}\label{fig:mov_sen_mov_tar_all}
	\end{subfigure}
	\caption{(a)-(d) The $1^\text{st}$ UGV's individual PDFs at different times; (e) All UGVs' individual PDFs at time $40$.}
	\label{fig:mov_sen_mov_tar}
\end{figure}

\todohere{change simulation plots}
This section simulates two scenarios of target localization to demonstrate the effectiveness of LIFO-BDF. 
In all scenarios, six UGVs are utilized and each UGV is equipped with a binary sensor. 
...
The positions of six static UGVs are shown as stars and square in \cref{fig:sta_sen_sta_tar}.
...
This paper presents a measurement dissemination-based distributed Bayesian filtering (DBF) method for a multi-UGV network, utilizing the Latest-In-and-Full-Out (LIFO) protocol for observation exchange.
% for target search and tracking.
Different from statistics dissemination approaches that transmit posterior distributions or likelihood functions, each UGV under LIFO only exchanges with neighboring UGVs a full communication buffer consisting of latest available measurements,
...
For the static target, each UGV locally fuses the newly received observations while for the moving target, a record set of historical observations is stored and updated. 
%Upon obtaining the latest available observations of all UGVs, an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps. 
...
%With fixed communication topology, LIFO guarantees the global dissemination of all UGVs' observations over the network only via local exchange of observations among neighbors.
...
%For the static target, each UGV locally fuses the latest knowledge of all UGVs' observations by only considering the updating step of the Bayesian filter. 
%For the moving target, each UGV maintains a triangle matrix of historical observations and an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps upon obtaining the latest available observations of all UGVs. 
