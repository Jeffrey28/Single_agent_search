...
This paper presents a local exchange protocol of observations, i.e., Latest-In-and-Full-Out (LIFO), for distributed Bayesian filters (DBF) in a multi-robot network.
...
The consistency of LIFO-based DBF is proved that individual PDF of each robot converges in probability to the true target position when the number of observations tends to infinity.
...
A consensus of individual estimates is achieved by solely communicating statistics and/or observations to its local neighborhood.
...
In consensus-based distributed filters, the robots diffuse statistics among neighbors, via which global agreement of the statistics is achieved by using consensus protocols \cite{olfati2007consensus,ren2005consensus,jadbabaie2003coordination}.
...
\todoQ{Though approximating statistics with parametric models, such as Gaussian Mixture Models \cite{sheng2005distributed}, can significantly reduce communication burden, such manipulation sacrifices accuracy of filtering due to the approximation.}
...
(2)	The corresponding DBF ensures consistency of individual posterior distributions, which also implies the consensus of target PDFs.
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\begin{equation*}
		%$\begin{array}{c}
		z^j_{k^i_j}=\emptyset,\; k^i_j=0,\;j=1,\dots,N
		%\end{array}$\\
		\end{equation*}
...
For a fixed and undirected network of $N$ robots, LIFO uses the shortest path(s) between $i^\text{th}$ and $j^\text{th}$ robot to exchange observation, whose length is the delay $\tau_{i,j}$ between them.
\end{prop}
\begin{proof} 
...
\begin{rem}
	...
	On the contrary, the length of transmitted data for a statistics dissemination approach \todoQ{that transmits unparameterized posterior distributions of likelihood functions} is $O(M^2)$, which is in the order of environment size. 
...
\begin{rem}
	...
	Therefore, except storing individual PDFs, all historical CBs can be discarded and only current-step CB is stored in robot memory, which means that the size of occupied memory is $O(N)$. 
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\State\textbf{(4)} Store the first-step virtual PDF as the individual PDF for time $(k-N+1)$
		\begin{equation*}
		P^i_{pdf}(x^T_{k-N+1}|z^1_{1:k-N+1},\dots,z^N_{1:k-N+1})=P^i_{virt}(x^T_{k-N+1}).
		\end{equation*}
	\end{algorithmic}
...
\begin{thm}\label{thm:LIFO-dbf-sta-tar}
	If robots are static, each individual PDF converges to $ X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.
	\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P^i_{pdf}(x^T\in X^T_{eq}|\mathbf{z}^i_{1:k})=
%	\begin{cases}
%	1 & \text{if}\; i=1\\
%	0 & \text{if}\; i\neq 1
%	\end{cases},\;
...
Since ${P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})}e^{c_Mk}{\longrightarrow}0$, \Cref{eqn:lim3} implies
...
\begin{thm}\label{thm:LIFO-dbf-mov-tar}
	If robots move within a collection of finite positions, each individual PDF by LIFO-DBF converges to $X^T_{eq}$ in probability when the number of observations tends to infinity, i.e.,
	\begin{equation*}
		\lim\limits_{k\rightarrow \infty}P^i_{pdf}(x^T\in X^T_{eq}|\mathbf{z}^i_{1:k})\longrightarrow
%		\begin{cases}
%		1 & \text{if}\; i=1\\
%		0 & \text{if}\; i\neq 1
%		\end{cases},\;
...
For the former, it is easy to know that their contribution to \Cref{eqn:lim} is zero.
...
\medskip
\begin{rem}
	The consistency means that all individual PDFs converge in probability to the same $X^T_{eq}$, thus the consensus is also guaranteed.
...
The second scenario subsequently deals with six moving robots for localizing a single moving target. 
...
As more observations are received, the posterior individual PDF converges to the true location of the target (\cref{fig:sta_sen_sta_tar_sing4}), which accords with the consistency of LIFO-DBF.
...
This paper presents a Latest-In-and-Full-Out (LIFO) protocol for measurement dissemination-based distributed Bayesian filters (DBF) in a multi-robot network.
% for target search and tracking.
Different from statistics dissemination approaches that transmit posterior distributions or likelihood functions, each robot under LIFO only receives the latest available measurements and then broadcasts full communication buffer to its direct neighborhood by single-hopping, which significantly reduces the transmission burden of each pair from the order of environmental size to that of robot number.
...
For the static target, each robot locally fuses the newly received observations while for the moving target, a triangular matrix of historical observations is stored and updated. 
%Upon obtaining the latest available observations of all robots, an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps. 
The consistency of LIFO-based DBF is proved utilizing the law of large numbers, which ensures that individual PDF of each robot converges in probability to the true target position when the number of observations tends to infinity.
%the agreement between robots' estimated target position and the actual position.
%The effectiveness of this method is demonstrated by simulations.
