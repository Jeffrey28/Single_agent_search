...
%	Generally, this scheme can be further categorized into two types: leader-based and consensus-based. In the former, statistics is sequentially passed and updated along a path formed by active UGVs, called leaders. Only leaders perform filtering based on its own measurement and received measurements from local neighbors \cite{sheng2005distributed}. In the latter, every UGV diffuses statistics among neighbors, via which global agreement of the statistics is achieved by using consensus protocols \cite{olfati2007consensus,ren2005consensus,jadbabaie2003coordination}. 
	...
	However, such a manipulation increases the computation burden of each agent and sacrifices filtering  accuracy due to approximation.
	...
	A recent work was conducted by Djuric et al. (2011), who proposed to broadcast raw measurements to other agents, and therefore each agent has a complete set of observations of other UGVs for executing particle filtering \cite{djuric2011non}.  A shortcoming of aforementioned works is that their communication topologies are assumed to be a fixed and complete graph that every pair of distinct UGVs is constantly connected by a unique edge. In many real applications,the  interaction topology may change dynamically due to unreliable links, external disturbances and/or range limits \cite{xiao2008asynchronous}.
...
%	If the information is being exchanged by direct sensing, the locally visible neighbors of a vehicle will likely change over time. 
	...
	The Bayesian filter is run locally on each UGV based on its own and received observations via single-hopping to estimate the target position.
...
%	Let $\bar{G}=\left\lbrace G_1,G_2,\dots,G_L\right\rbrace $ denote the set of all possible simple and undirected graphs defined for the network of UGVs.
	...
	$z^j_{k^i_j}$ is stored in the CB of ${i^\text{th}}$ UGV, where $k^i_j$ is the latest observation time of ${j^\text{th}}$ UGV available to ${i^\text{th}}$ UGV by time $k$. Due to the communication delay, $k^i_j<k, \forall j\neq i$ and $k^i_i=k$ always holds.
...
%	\todohere{change the figure to be a switching topology. change the contents here accordingly}
	...
	There are two types of topologies, the first only UGV $1$ and UGV $2$ or UGV $2$ and UGV $3$ can communicate with each other.
...
%	, which means under LIFO each UGV has a maximum delay of 2 steps for receiving observations from other UGVs; 
	...
	Extending the these facts to a network of $N$ UGVs, we have the following proposition:
	%\medskip
	
	\begin{figure}%[thpb]
		\centering
		\includegraphics[width=0.45\textwidth]{figures/data_exchange_switch}
		\caption{Example of LIFO with three UGVs using switching line interaction topologies. The hollow arrow represents a communication link between two UGVs.\todohere{change 'robot' to UGV; add topology for k=0 and 7.}}
		\label{fig:LIFO}
	\end{figure}
...
%		\todohere{may rephrase the following sentence}
		...
		then arbitrary pair of UGVs can exchange observations under LIFO. In addition, the delay between the pair of UGVs is no greater than \small$(N-1)T_u$\normalsize, where \small$T_u=\sup\limits_{m=1,2,\dots}\left( k_{m+1}-k_m\right) T$ \normalsize is the upper bound of interval lengths.
	\end{prop}
...
%		 $i^\text{th}$ and $j^\text{th}$ UGV.
	...
	\begin{cor}\label{cor1}
		...
		Additionally, each element keeps updating every finite period of time.
	\end{cor}
	\begin{proof}
	...
	\end{equation}\normalsize
	where $x^T$ denotes the target position; $x^{R,i}$ is the UGV position.
...
%	results of LIFO-DBF under the condition of the interaction topology described in \cref{prop1}.
	...
	\begin{proof}	
		...
		It is known from \Cref{cor1} that the cardinality of $\mathscr{K}^i_{j,k}$ has following property: \small$k-(N-1)T_u<|\mathscr{K}^i_{j,k}|\leq k$\normalsize.
...
%	\end{rem}
	...
	\begin{figure}%[thpb]
		...
		\begin{subfigure}[b]{0.17\textwidth}
			...
			\caption{Step 1}\label{fig:sta_sen_sta_tar_sing1_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
		...
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
			\includegraphics[width=\textwidth]{figures/sta_sen_sta_tar_single_2_1}
			\caption{Step 1}\label{fig:sta_sen_sta_tar_sing2_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
		...
		\begin{subfigure}[b]{0.17\textwidth}
			...
			\caption{Step 1}\label{fig:sta_sen_sta_tar_sing3_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
...
%		\end{subfigure}
		\caption{Individual PDFs for two changing interaction topologies: (b)-(e) The $1^\text{st}$ UGV's individual PDFs at different times; (f)-(i) The $2^\text{nd}$ UGV's individual PDFs; (j)-(m) The $5^\text{nd}$ UGV's individual PDFs. The cross stands for the target.\todohere{four plots combine into one single plot. PDF evolution}}
%			The square denotes the current UGV and stars represent other UGVs. The cross stands for the target.}
	...
	\begin{figure}%[thpb]
		...
		~
		\begin{subfigure}[b]{0.17\textwidth}
			\includegraphics[width=\textwidth]{figures/sta_sen_sta_tar_2_single_1_1}
			\caption{Step 1}\label{fig:sta_sen_sta_tar_2_sing1_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
		...
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
			\includegraphics[width=\textwidth]{figures/sta_sen_sta_tar_2_single_2_1}
			\caption{Step 1}\label{fig:sta_sen_sta_tar_2_sing2_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
		...
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
			\includegraphics[width=\textwidth]{figures/sta_sen_sta_tar_2_single_5_1}
			\caption{Step 1}\label{fig:sta_sen_sta_tar_2_sing3_1}
		\end{subfigure}
		\begin{subfigure}[b]{0.17\textwidth}
...
%		\end{subfigure}
		\caption{Individual PDFs for three switching interaction topologies: (b)-(e) The $1^\text{st}$ UGV's individual PDFs; (f)-(i) The $2^\text{nd}$ UGV's individual PDFs; (j)-(m) The $5^\text{nd}$ UGV's individual PDFs. }
	...
	The union of both collections are designed to be jointly connected.
	In both scenarios, topologies appear alternatively such that they can be said to be connected frequently enough.
