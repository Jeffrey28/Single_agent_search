...
\todohere{directly write z\_cb here}
...
$P^i_{pdf}(x^T_{k}|\mathbf{z}^i_{1:k})$, where $\mathbf{z}^i_{1:k}$ denotes the set of observations by $i^\text{th}$ UGV and by UGVs in $\mathcal{Q}_i$ that have been stored in CB of $i^\text{th}$ UGV by time $k$.
