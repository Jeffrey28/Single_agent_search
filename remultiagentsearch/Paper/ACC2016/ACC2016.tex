...
This paper presents measurement dissemination-based distributed Bayesian filtering (DBF) approach for a multi-robot network, utilizing the proposed Latest-In-and-Full-Out (LIFO) local exchange protocol of observations.
% for target search and tracking.
Different from statistics dissemination approaches that transmit posterior distributions or likelihood functions, each robot under LIFO only receives the latest available measurements and then broadcasts full communication buffer to its neighborhood, which significantly reduces the transmission burden between each pair of robots from the order of environmental size to that of robot number.
...
The consistency of LIFO-based DBF is proved that estimated target position of each robot converges in probability to the true target position when the number of observations tends to infinity.
...
As a generic filtering scheme for nonlinear systems and arbitrary noise distributions, distributed Bayesian filter (DBF) has received increasing interest during past years \cite{bandyopadhyay2014distributed,julian2012distributed}, which is the focus of this study.
...
In NB-based approaches, each robot executes a distributed filter to estimate individual posterior distribution. 
A consensus of individual estimates is achieved by solely communicating statistics and/or observations within local neighbors of each robot.
%Only communication between neighboring agents is allowed.
The NB-based methods have become popular in recent years since such approaches do not require complex routing protocols or global knowledge about the network and therefore are robust to changes in the network topology and to link failures.
...
In the former, environment status is sequentially passed and updated along a path formed by active robots, called leaders.
...
Hlinka et al. (2012) proposed a distributed method for computing an approximation of the joint (all-sensors) likelihood function by means of weighted-linear-average consensus algorithm when local likelihood functions belong to the exponential family of distributions \cite{hlinka2012likelihood}.
...
This paper extends existing works by introducing a Latest-In-and-Full-Out (LIFO) protocol for distributed Bayesian filters (DBF) of networked robots. 
...
The main benefit of LIFO is on the reduction of communication burden, with the transmission data volume scaling linearly with the robot number, while a statistics dissemination-based strategy can suffer from the order of environmental size.
...
The Bayesian filter is run locally on each robot based on its own and received observations via single-hopping. 
...
\end{equation}\normalsize
where $x^T_k$ denotes the target position; $x^R_k$ is the robot position.
...
For the purpose of simplicity, we will not explicitly write $x^R_k$ when no confusion may occur.
...
\begin{rem}
	Given the knowledge of current target position, current observation of each robot is conditionally independent from its own past observations and those of other robots.
\end{rem}

\begin{rem} 
	The proposed LIFO protocol is applicable for both homogeneous and heterogeneous binary sensors. 
...
In addition to direct neighborhood, another set called \textit{available neighborhood} is defined as $\mathcal{Q}_i$, which contains indices of robots whose observations are received by the $i^\text{th}$ robot given a specific observation exchange protocol. 
...
\begin{figure}%[thpb]
	\centering
	\includegraphics[width=0.4\textwidth]{figures/com_topo}
...
The generic distributed Bayesian filter (DBF) is introduced in this section, which was also stated in \cite{bandyopadhyay2014distributed} and \cite{julian2012distributed}. 
...
For the same topology in \Cref{prop1}, all elements in $\mathbf{z}^i_k$ under LIFO become filled when $k\geq N$, i.e., $\mathcal{Q}_i = \left\lbrace1,\dots,N \right\rbrace $.	
\end{cor}
\begin{proof}
...
\begin{rem}
	...
	On the contrary, the length of transmitted data for a statistics dissemination approach that transmits unparameterized posterior distributions or likelihood functions is $O(M^2)$, which is in the order of environment size. 
...
\begin{figure}%[thpb]
	...
	\caption{Example of LIFO-DBF for $1^\text{st}$ robot at time $k$. The stored individual PDF is $ P^1_{pdf}(k-N)$.
	The robot first calculates $ P^1_{virt}(k-N+1)$ using DBF and stores it as $ P^1_{pdf}(k-N+1)$. Repeating DBF until obtaining $ P^1_{pdf}(k)$, which is then used as the target PDF estimation of $1^\text{st}$ robot at time $k$.
	$\Omega^1_{\xi}=\left\lbrace 1,2,\dots,N+1-\xi\right\rbrace $, $\xi=1,\dots,N$.}
	\label{fig:LIFO-DBF}
\end{figure}
...
\begin{rem}
	...
	On the contrary, for the moving target, each robot needs to store a triangular matrix of historical observation (except current step CB) with size of $O(N^2)$ and an individual PDF with size $O(M^2)$, which means that the size of occupied memory in each robot is $O(M^2+N^2)$.
\end{rem}
...
Considering $S$ is finite and $x^{T^*}$ is the true location, define an \textit{equivalent-location} set $X^T_{eq}\subseteq S$ such that 
\small\begin{equation*}
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\State(3.3) Only when $\xi=1$, store the first-step virtual PDF as the individual PDF for time $(k-N+1)$
		\begin{equation*}
		P^i_{pdf}(x^T_{k-N+1}|z^1_{1:k-N+1},\dots,z^N_{1:k-N+1})=P^i_{virt}(x^T_{k-N+1}).
		\end{equation*}
		
		\State\textbf{(4)} Individual PDF of $i^\text{th}$ robot at time $k$ is
		$P^i_{pdf}(x^T_{k}|\mathbf{z}^i_{1:k})=P^i_{virt}(x^T_k)$.		
	\end{algorithmic}
\end{algorithm}
...
\begin{thm}\label{thm:LIFO-dbf-sta-tar}
	When robots are static, the set of estimated target position of each robot converges to $ X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^T\in X^T_{eq}|\mathbf{z}^i_{1:k})=
%	\begin{cases}
%	1 & \text{if}\; i=1\\
%	0 & \text{if}\; i\neq 1
%	\end{cases},\;
...
\small\begin{subequations}
	\begin{align*}
		...
		&=\frac{P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k_j}P(z^j_l|x^T)}{\sum\limits_{x^T\in S}P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k_j}P(z^j_l|x^T)},
	\end{align*}
\end{subequations}\normalsize
...
Comparing $P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})$ with $P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})$ yields
...
\small\begin{equation*}
P(z^j_k|x^T)=p^{z^j_k}_j(1-p_j)^{1-z^j_k}
\end{equation*}\normalsize
where $p_j=P(z^j_k=1|x^T)$. 
Utilizing the facts: (1) $z^j_l$ are conditionally independent samples from $B(1,p_j^*)$ and (2) $k-N<k_j\leq k$, the law of large numbers yields
\small\begin{equation*}
%\lim\limits_{k\rightarrow \infty}
\frac{1}{k}\sum\limits_{l=1}^{k_j}z^j_l\overset{P}{\longrightarrow}p^*_j,\quad 
%\lim\limits_{k\rightarrow\infty}
\frac{1}{k}(k_j-\sum\limits_{l=1}^{k_j}z^j_l)\overset{P}{\longrightarrow}1-p^*_j
\end{equation*}\normalsize
where $p_j^*=P(z^j_k=1|x^{T^*})$ and ``$\overset{P}{\longrightarrow}$" denotes ``convergence in probability''.
Then, 
...
% \lim\limits_{k\rightarrow \infty}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})}\overset{P}{\longrightarrow}c(x^T)
...
\frac{P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})e^{c(x^T)k}}\overset{P}{\longrightarrow}1
\end{equation}\normalsize

Define the set $\bar{X}_{eq}^T=X^T\setminus X^T_{eq}$ and $c_M=\max\limits_{x^T\in\bar{X}^T_{eq}}c(x^T)$.
Then $c_M<0$.
Summing \Cref{eqn:lim2} over $\bar{X}_{eq}^T$ yields
\small\begin{equation}\label{eqn:lim3}
\frac{\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})e^{\left[c_M-c(x^T)\right]k}}{{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})}e^{c_Mk}}\overset{P}{\longrightarrow} |\bar{X}_{eq}^T|
\end{equation}\normalsize
where $|\bar{X}_{eq}^T|$ denotes the cardinality of $\bar{X}_{eq}^T$.

Since $c_M<0$,  ${P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})}e^{c_Mk}{\longrightarrow}0$, \Cref{eqn:lim3} implies
\begin{equation}\label{eqn:lim4}
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})e^{\left[c_M-c(x^T)\right]k}\overset{P}{\longrightarrow}0
\end{equation}

Utilizing the relation 
\begin{equation*}
0\leq P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})\leq P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})e^{\left[c_M-c(x^T)\right]k},
\end{equation*}
it can be derived from \Cref{eqn:lim4} that
\small\begin{equation*}
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})\overset{P}{\longrightarrow} 0
\end{equation*}\normalsize

Therefore,
\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^T\in X^T_{eq}|\mathbf{z}^i_{1:k})=\lim\limits_{k\rightarrow \infty}\sum\limits_{x^T\in X_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^i_{1:k})=1
\end{equation*}\normalsize
%\begin{enumerate}
%	\item When $p_j\neq p^*_j$, i.e., $x^T\notin X^T_{eq}$,
%	\begin{align*}
...
\begin{thm}\label{thm:LIFO-dbf-mov-tar}
	If robots move within a collection of finite positions, the set of estimated target position of each robot converges to $X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	\small\begin{equation*}
		\lim\limits_{k\rightarrow \infty}P(x^T\in X^T_{eq}|\mathbf{z}^i_{1:k})=
%		\begin{cases}
%		1 & \text{if}\; i=1\\
%		0 & \text{if}\; i\neq 1
%		\end{cases},\;
...
For the former, it is easy to know that their contribution to \Cref{eqn:cmp2} is zero when $k\rightarrow \infty$.
Therefore, \Cref{eqn:cmp2} can be reduced to only consider infinite-observation spots, which is similar to \Cref{thm:LIFO-dbf-sta-tar}.
\end{proof}
\medskip
\begin{rem}
	The consistency means that all individual PDFs converge to the same distribution, thus the consensus is also guaranteed.
...
In this work, the robot positions are randomly generated at each time in order to demonstrate the effectiveness of LIFO-DBF.
...
This paper presents measurement dissemination-based distributed Bayesian filters (DBF) for a multi-robot network, utilizing the Latest-In-and-Full-Out (LIFO) protocol for observation exchange.
% for target search and tracking.
Different from statistics dissemination approaches that transmit posterior distributions or likelihood functions, each robot under LIFO only receives the latest available measurements and then broadcasts full communication buffer to its direct neighborhood by single-hopping, which significantly reduces the transmission burden between each pair of robots from the order of environmental size to that of robot number.
...
For the static target, each robot locally fuses the newly received observations while for the moving target, a triangular matrix of historical observations is stored and updated. 
%Upon obtaining the latest available observations of all robots, an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps. 
The consistency of LIFO-based DBF is proved utilizing the law of large numbers, which ensures that estimated target position of each robot converges in probability to the true target position when the number of observations tends to infinity.
%the agreement between robots' estimated target position and the actual position.
%The effectiveness of this method is demonstrated by simulations.
