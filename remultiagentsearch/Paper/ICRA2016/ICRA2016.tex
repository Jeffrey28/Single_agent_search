...
\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^{T^*}|\mathbf{z}^{i}_{1:k})=1-\lim\limits_{k\rightarrow \infty}\sum\limits_{x^T\in \bar{X}^T}P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})=1
\end{equation*}\normalsize
%\begin{enumerate}
%	\item When $p_j\neq p^*_j$, i.e., $x^T\notin X^T_{eq}$,
%	\begin{align*}
...
%	When $X^T_{eq}$ only contains $x^{T^{*}}$, consistency means the estimated target position converges to true target position in probability. Additionally, 
	...
	To the best knowledge of authors, there is no proof of consistency that guarantees the agreed PDF is close to the true target PDF.
%	 of individual PDFs.
