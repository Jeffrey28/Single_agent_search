...
$P^i_{pdf}(x^T_{k}|\mathbf{z}^i_{1:k})$, where $\mathbf{z}^i_{1:k}$ denotes the set of observations made by $i^\text{th}$ robot and observations by robots in $\mathcal{Q}_i$ that are transmitted to $i^\text{th}$ robot up to time $k$.
...
\end{align*}\normalsize
where $P^i_{pdf}(x^T_k|\mathbf{z}^i_{1:k})$ is called posterior individual PDF; $P(z^i_k|x^T_k)$ is the likelihood function of observation $z^i_k$, described in \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0}.
...
According to \Cref{cor2}, $\mathbf{z}^{i}_{1:k}=\mathbf{z}^{CB,i}_{1:k}=\left[ z^1_{1:k^i_1},\dots,z^N_{1:k^i_N}\right]$.
...
%	the set of estimated target position of each robot converges to $ X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	...
	P(x^T\in X^T_{eq}|\mathbf{z}^{i}_{1:k})=
%	\begin{cases}
%	1 & \text{if}\; i=1\\
%	0 & \text{if}\; i\neq 1
%	\end{cases},\;
...
\frac{\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})e^{\left[c_M-c(x^T)\right]k}}{{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}e^{c_Mk}}\overset{P}{\longrightarrow} |\bar{X}_{eq}^T|
\end{equation}\normalsize
...
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})e^{\left[c_M-c(x^T)\right]k}\overset{P}{\longrightarrow}0
\end{equation}
...
\small\begin{equation*}
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})\overset{P}{\longrightarrow} 0
\end{equation*}\normalsize

Therefore,
\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^T\in X^T_{eq}|\mathbf{z}^{i}_{1:k})=\lim\limits_{k\rightarrow \infty}\sum\limits_{x^T\in X_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})=1
\end{equation*}\normalsize
%\begin{enumerate}
%	\item When $p_j\neq p^*_j$, i.e., $x^T\notin X^T_{eq}$,
%	\begin{align*}
