...
Different from statistics dissemination approaches that transmit posterior distributions or likelihood functions, each robot under LIFO only exchanges a full communication buffer consisting of latest available measurements with neighboring robots,
...
In NB-based approaches, certain robots execute distributed filters to estimate individual posterior distribution. 
A consensus of individual estimates is achieved by solely communicating statistics and/or observations within local neighbors of these robots.
...
The combination of \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0} becomes a binary sensor model parameterized by $x^T_k$ and $x^{R,i}_k$. 
...
The individual PDF of $i^\text{th}$ robot at time $k$ is defined as $P^i_{pdf}(x^T_{k}|z^i_{1:k},z^{\mathcal{Q}_i}_{1:k})$, where $z^i_{1:k}$ denotes the set of observations made by $i^\text{th}$ robot from time $1$ through $k$ and $z^{\mathcal{Q}_i}_{1:k}$ means the set of observations by robots in $\mathcal{Q}_i$ that are transmitted to $i^\text{th}$ robot.
...
Each robot stores last-step individual PDF, i.e., $P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k-1})$. 
...
P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})=K_iP^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k-1})\prod\limits_{j=1}^{N}P(z^j_{k^i_j}|x^T)
\end{equation}\normalsize
where
\small\begin{equation*}
K_i=1/\int P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k-1})\prod\limits_{j=1}^{N}P(z^j_{k^i_j}|x^T)dx^T
\end{equation*}\normalsize
...
Instead of storing last-step PDF, at time $k$ each robot maintains an individual PDF of time $(k-N)$ and a matrix of historical observations from time $(k-N+1)$ to $k$. 
...
\begin{figure}%[thpb]
	...
	\caption{Example of LIFO-DBF for $1^\text{st}$ robot at time $k$. 
		(a) line topology of $N$ networked robots;
		(b) LIFO-DBF procedure: The stored individual PDF is $ P^1_{pdf}(k-N)$.
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		$P^i_{pdf}(x^T_{k}|\mathbf{z}^{CB,i}_{1:k})=P^i_{virt}(x^T_k)$.		
	\end{algorithmic}
\end{algorithm}
...
\begin{thm}\label{thm:LIFO-dbf-sta-tar}
	When robots are static, the probability of estimated target position belonging to $ X^T_{eq}$ converges to one, i.e.,
%	the set of estimated target position of each robot converges to $ X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	...
	P(x^T\in X^T_{eq}|\mathbf{z}^{CB,i}_{1:k})=
%	\begin{cases}
%	1 & \text{if}\; i=1\\
%	0 & \text{if}\; i\neq 1
%	\end{cases},\;
...
\begin{proof}	
Considering the conditional independence of observations for given $x^T\in S$, the batch form of DBF at $k^\text{th}$ step is:
\small\begin{subequations}
	\begin{align*}
		P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})&=P^i_{pdf}(x^T|z^1_{1:k^i_1},\dots,z^N_{1:k^i_N})\\
...
Comparing $P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})$ with $P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})$ yields
\small\begin{equation}\label{eqn:cmp}
\frac{P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})}=\frac{P^i_{pdf}(x^T)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^T)}{P^i_{pdf}(x^{T^*})\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^{T^*})}
\end{equation}\normalsize

Take the logarithm of \Cref{eqn:cmp} and average it over $k$ steps:
\small\begin{equation}\label{eqn:cmp_log}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})}=\frac{1}{k}\ln\frac{P^i_{pdf}(x^T)}{P^i_{pdf}(x^{T^*})}+\sum\limits_{j=1}^{N}\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}.
\end{equation}\normalsize
...
% \lim\limits_{k\rightarrow \infty}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})}\overset{P}{\longrightarrow}c(x^T)
...
\frac{P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})e^{c(x^T)k}}\overset{P}{\longrightarrow}1
\end{equation}\normalsize

Define the set $\bar{X}_{eq}^T=S\setminus X^T_{eq}$ and $c_M=\max\limits_{x^T\in\bar{X}^T_{eq}}c(x^T)$.
Then $c_M<0$.
Summing \Cref{eqn:lim2} over $\bar{X}_{eq}^T$ yields
\small\begin{equation}\label{eqn:lim3}
\frac{\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})e^{\left[c_M-c(x^T)\right]k}}{{P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})}e^{c_Mk}}\overset{P}{\longrightarrow} |\bar{X}_{eq}^T|
\end{equation}\normalsize
where $|\bar{X}_{eq}^T|$ denotes the cardinality of $\bar{X}_{eq}^T$.

Since $c_M<0$,  ${P^i_{pdf}(x^{T^*}|\mathbf{z}^{CB,i}_{1:k})}e^{c_Mk}{\longrightarrow}0$, \Cref{eqn:lim3} implies
\begin{equation}\label{eqn:lim4}
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})e^{\left[c_M-c(x^T)\right]k}\overset{P}{\longrightarrow}0
\end{equation}

Utilizing the relation 
\begin{equation*}
0\leq P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})\leq P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})e^{\left[c_M-c(x^T)\right]k},
\end{equation*}
it can be derived from \Cref{eqn:lim4} that
\small\begin{equation*}
\sum\limits_{x^T\in \bar{X}_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})\overset{P}{\longrightarrow} 0
\end{equation*}\normalsize

Therefore,
\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^T\in X^T_{eq}|\mathbf{z}^{CB,i}_{1:k})=\lim\limits_{k\rightarrow \infty}\sum\limits_{x^T\in X_{eq}^T}P^i_{pdf}(x^T|\mathbf{z}^{CB,i}_{1:k})=1
\end{equation*}\normalsize
%\begin{enumerate}
%	\item When $p_j\neq p^*_j$, i.e., $x^T\notin X^T_{eq}$,
%	\begin{align*}
...
\begin{thm}\label{thm:LIFO-dbf-mov-tar}
	If robots move within a collection of finite positions, the probability of estimated target position belonging to $ X^T_{eq}$ converges to one, i.e.,
%	the set of estimated target position of each robot converges to $X^T_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	\small\begin{equation*}
...
\medskip
\begin{rem}
	When $X^T_{eq}$ only contains $x^{T^{*}}$, consistency means the estimated target position converges to true target position in probability. Additionally, consistency implies that all individual PDFs converge to the same distribution, thus the consensus is also guaranteed.
	...
	To the best knowledge of authors, there is no proof of consistency that the agreed PDF is close to the true target PDF.
%	 of individual PDFs.
