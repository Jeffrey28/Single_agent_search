...
%         end
        ...
        function labeledRes = labelResult(res,fctname,N) %this,
            ...
            % x
            for ii = 1:N+1
                varHeader = [varHeader,{sprintf('x(1,%d)',ii-1)},{sprintf('x(2,%d)',ii-1)}];
            end
            %x_pred
            for ii = 1:N
                varHeader = [varHeader,{sprintf('x_pred(1,%d)',ii)},{sprintf('x_pred(2,%d)',ii)}];
            end
            % P
            for ii = 1:N+1
                varHeader = [varHeader,{sprintf('P(1,1,%d)',ii-1)},{sprintf('P(2,1,%d)',ii-1)},...
                    {sprintf('P(1,2,%d)',ii-1)},{sprintf('P(2,2,%d)',ii-1)}];
            end
            % P_pred
            for ii = 1:N
                varHeader = [varHeader,{sprintf('P_pred(1,1,%d)',ii)},{sprintf('P_pred(2,1,%d)',ii)},...
                    {sprintf('P_pred(1,2,%d)',ii)},{sprintf('P_pred(2,2,%d)',ii)}];
            end
            % K
            for ii = 1:N
                varHeader = [varHeader,{sprintf('K(1,1,%d)',ii)},{sprintf('K(2,1,%d)',ii)},...
                    {sprintf('K(1,2,%d)',ii)},{sprintf('K(2,2,%d)',ii)}];
            end
