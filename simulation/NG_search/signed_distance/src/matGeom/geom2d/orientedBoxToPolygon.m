...
x   = obox(1);
y   = obox(2);
lf  = obox(3);  % easier to compute with w and h divided by 2
lr  = obox(4);
w_half  = obox(5) / 2;
if length(obox) > 5
    theta = obox(6);
...
df = sqrt(w_half^2 + lf^2);
dr = sqrt(w_half^2 + lr^2);
alphaf = atan(w_half/lf);
alphar = atan(w_half/lr);

tx = [x x x x]'+[df*cos(alphaf-theta),df*cos(theta+alphaf),-dr*cos(-theta+alphar),-dr*cos(theta+alphar)]';
ty = [y y y y]'+[-df*sin(alphaf-theta),df*sin(theta+alphaf),dr*sin(-theta+alphar),-dr*sin(theta+alphar)]';
...
%this version has input l instead of l_f, l_r
