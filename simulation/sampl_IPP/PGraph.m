% original file from Peter Corke's robotics toolbox
% adpated by Chang Liu
% 4/29/26
...
%   self-added:
%   g.getvlist()           return the vertexlist
%   g.findEdge(v1,v2)      return the edge connecting v1 and v2
%
% Display::
%
...
classdef PGraph < handle
    ...
    properties (SetAccess=private, GetAccess=private)
        ...
        userinfodata    %  per vertex info data, cell array
    ...
    methods
        ...
        function u = setinfodata(g, v, u)
        %PGraph.setinfodata Set user data for node
        %
        % G.setdata(V, U) sets the user data of vertex V to U which can be of any 
        % type such as a number, struct, object or cell array.
        %
        ...
        function e = findEdge(g, v1, v2)
        %PGraph.findEdge Find the edge id e for the edge [v1;v2]
        %
        % E = G.edges(V) is a vector containing the id of all edges connected to vertex id V.
        %
        % See also PGraph.edgedir.
            e = find((g.edgelist(1,:) == v1) & (g.edgelist(2,:) == v2));
        end
    ...
    % private methods
        ...
        function d = distance_metric(g, x1, x2)
            ...
            switch g.measure
                ...
                case 'Info' % information gain
                    d1 = colnorm( bsxfun(@minus, x1, x2) );
                    d2 = d1*2;
                    d = (d1+d2)/2;
