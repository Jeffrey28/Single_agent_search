...
%% this part is my original implementation. It gives different answers from what Junkai and Chungyen did,
% So I adopted Junkai's code for this part. However the result is still a
% little different... Also, when I run Chungyen's code on my computer, its
% result is the same as the result generated by this code...
%{
iter = 0;
while true    
    iter = iter+1;
    ...
    hinge = @(x) sum(max(x,0));
    abssum = @(x) sum(abs(x));
    if (hinge(g(x))+abssum(h(x)) >= cfg.cnt_tolerance)
    ...
    while  true
        ...
        while true 
            ...
            cvx_begin quiet
				...
				%YOUR CODE HERE
				minimize fval+fgrad*(xp-x)+fquadlin(xp)+penalty_coeff*(hinge(gval+gjac*(xp-x))+abssum(hval+hjac*(xp-x)))
                subject to
