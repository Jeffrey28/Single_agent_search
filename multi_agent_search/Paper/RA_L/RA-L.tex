...
\author{Chang~Liu,~\IEEEmembership{Student Member,~IEEE,}
	Shengbo~Eben~Li,%~\IEEEmembership{Fellow,~OSA,}
	...
	\thanks{Shengbo Eben Li is with the Department of Automotive Engineering, Tsinghua University, Beijing, 100084, China. Email: {\tt\small lisb04@gmail.com.}}
}% <-this % stops a space
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\caption{(a)-(d) The $1^\text{st}$ robot's individual PDFs at different times; (e)-(h) The $3^\text{rd}$ robot's individual PDFs at different times; (i)-(l) The $5^\text{th}$ robot's individual PDFs at different times. Robots are numbered counterclockwise, with the left-most robot being the $1^\text{st}$. The square denotes the current robot and stars represent other robots. The cross stands for the target.}
...
\begin{figure}%[thpb]
	...
	\caption{(a)-(d) The $2^\text{nd}$ robot's individual PDFs at different times; (e)-(h) The $4\text{th}$ robot's individual PDFs at different times; (i)-(l) The $6^\text{th}$ robot's individual PDFs at different times. Robots moves in circles and the trajectories are shown in red and green. The square and starts denote the current positions of robots. The target trajectory is shown as the black line.}
