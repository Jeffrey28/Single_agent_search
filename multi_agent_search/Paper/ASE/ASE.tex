...
\usepackage{algorithm,algpseudocode} % algorithm environemnt
...
\author{Chang~Liu,~\IEEEmembership{Student Member,~IEEE,}
	...
	\thanks{*The first two authors, C. Liu and S. Li, have equally contributed to this research. Please direct all correspondence to S. Li.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
The multi-robot network has attracted increasing interest in estimating environment states for various applications.
...
\begin{ntp}
Deploying teams of robots for collecting environment information has received increasing interest in both military and civilian applications, such as surveillance, reconnaissance, search and rescue.
This work focuses on distributed estimation of hidden target position in the environment.
One major challenge for effective distributed estimation is the limited communication range and resource of robots.
Traditional methods usually incur large transmission burden or assume that all robots can communicate with each other.
This paper presents a distributed estimation scheme that is based on local exchange of sensor measurements among neighboring robots.
The proposed scheme significantly reduces transmission burden and only relies on local communication, therefore suitable for applications in realistic situations.
%In addition, under the condition that communication between robots are bidirectional and unchanged, the proposed method ensures that all robots correctly localize the true position of the target, when enough observations are obtained.
Simulations using a team of robots for localizing static and moving target are conducted to demonstrate the effectiveness of the proposed method. 
In future research, we will address the distributed estimation when communication between robots is time-variant and unidirectional.
\end{ntp}

\begin{IEEEkeywords}
Robot, distributed estimation, multi-robot network, communication topology.
\end{IEEEkeywords}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{INTRODUCTION}
\IEEEPARstart{D}{istributed} filtering that focuses on using a group of networked robots to collectively infer environment status has been used for various applications, such as intruder detection \cite{schiff2006automated}, pedestrian tracking \cite{wang2007wlan} and \textcolor{black}{environmental monitoring \cite{song2005automating,pandey2015dynamic}}. 
...
Despite the popularity of statistics dissemination strategy, exchanging statistics can consume high communication resources \cite{chang2014communication}.
...
One potential remedy is to disseminate measurement instead of statistics among neighbors, which has not been fully exploited in robotics community so far.
...
\textcolor{black}{One pioneering work was done by Rosencrantz et al. (2003), who proposed a selective communication scheme that enabled individual robots to communicate the most informative observation to other entities.
However, the selection of transmitted observation requires exchange of posterior distributions, which can incur high communication cost \cite{rosencrantz2002decentralized}.}
Ribeiro et al. (2006) \textcolor{black}{proposed a distributed estimation method based on exchanging} quantized observations along with error-variance limits considering pragmatic signal models \cite{ribeiro2006bandwidth}.
...
The Bayesian filter is executed locally on each robot for target localization, based on its own and received observations via single-hopping communication.
...
\textcolor{black}{Note that if the target is static, such as in the application of indoor search for stationary objects \cite{kulich2014single}, Markov motion model is simplified to be}
\small\begin{equation*}
...
\end{equation}\normalsize
where $K_i$ is a normalizing factor:
%\small\begin{align*}
...
The following mathematical induction proves \Cref{prop1}.
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\small\begin{align*}
			%		l_\text{latest}&=\argmax_{l\in \mathcal{N}_i,\;i}\left\lbrace\left(k-1\right)^i_j,\left(k-1\right)^l_j  \right\rbrace\\
			l_{\text{latest},j}&=\argmax_{l\in \mathcal{N}_i\cup \{i\}}\left\lbrace\left(k-1\right)^l_j  \right\rbrace\\
			z^j_{k^i_j}&=z^j_{\left(k-1\right)^{l_{\text{latest,j},j}}_j},\; k^i_j=\left(k-1\right)^{l_{\text{latest},j}}_j. 
		\end{align*} \normalsize
...
\begin{figure}%[thpb]
	...
	\caption{Example of LIFO-DBF for $1^\text{st}$ robot at time $k$. 	 
		...
		Repeating DBF until obtaining $ P^1_{pdf}(k)$, which is then used as the individual PDF estimation of $1^\text{st}$ robot at time $k$.
...
\begin{rem}
	...
	Therefore, besides storing individual PDF \textcolor{black}{ with size $O(M^2)$}, only current-step CB is stored in robot memory and all historical CBs can be discarded, which means that the size of occupied memory is \textcolor{black}{$O(N+M^2)$}. 
...
This section proves consistency and consensus of LIFO-DBF for localizing static target are presented, including both static and moving robots.
...
\begin{proof}
Let $n^{i,k}_j$ denote the times that $i^\text{th}$ robot visits $j^\text{th}$ position by time $k$. Then, $\sum\limits_{j} n^{i,k}_j=k$. It is straightforward to see that $\exists n^{i,k}_j,$ such that $n^{i,k}_j\rightarrow \infty,$ as $k\rightarrow \infty$.
\end{proof}
\medskip
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\caption{Individual PDFs at different steps in ``static robots + static target'' scenario: $1^\text{st}$ robot (a)-(d); $3^\text{rd}$ robot (e)-(h); $5^\text{th}$ robot (i)-(l). Robots are numbered counterclockwise, with the left-most one being the $1^\text{st}$. The square denotes the robot, \textcolor{black}{whose individual PDF is plotted,} and stars represent other robots. The cross stands for the target.}
...
\begin{figure}%[thpb]
	...
	\caption{Individual PDFs at different steps in ``moving robots + moving target'' scenario: $2^\text{nd}$ robot (a)-(d); $4^\text{th}$ robot (e)-(h); $6^\text{th}$ robot (i)-(l). Robots move in circles shown in red (the robot of focus) and green (other robots). The square and stars denote current positions of robots. The trajectory of the target is represented by the black line.}
...
The entropy of individual PDF at $k$ is calculated using the following formula:
\begin{equation*}
H_k(x^T)=-\int P^i_\text{PDF}(x^T|z_{1:k})\log P^i_\text{PDF}(x^T|z_{1:k})dx^T.
\end{equation*}

\cref{fig:sta_sen_sta_tar_entropy} shows the decreasing entropy of individual PDFs over time.
...
In this scenario, the six robots are controlled to move in circular trajectories periodically while the target moves linearly. 
Readers interested in motion planning of robots for target localization can refer to \cite{tisdale2009autonomous,sujit2011self,morbidi2011estimation}. %,furukawa2006recursive
The target dynamics is given by a single-integrator model: $x^T_{k+1}=x^T_{k}+v\Delta T$,
%\small\begin{equation*}
...
\begin{figure}%[thpb]
	...
	\begin{subfigure}[b]{0.3\textwidth}
		...
		\caption{Static robots}\label{fig:sta_sen_sta_tar_entropy}
	\end{subfigure}	
	\begin{subfigure}[b]{0.3\textwidth}
		\includegraphics[width=\textwidth]{figures/mov_sen_mov_tar_entropy}
		\caption{Moving robots}\label{fig:mov_sen_mov_tar_entropy}
	\end{subfigure}
\caption{Entropy of target PDF.}
\end{figure}
...
For static target, each robot locally fuses the newly received observations while for moving target, a record set of historical observations is stored and updated. 
%Upon obtaining the latest available observations of all robots, an iterative Bayesian filtering procedure is applied that alternates between prediction and updating steps. 
...
The authors gratefully acknowledges the ONR for supporting the research described in this paper. 
This work is also partially supported by NSF China with grant 51575293.
They would also like to thank Dr. Thierry Peynot from Queensland University of Technology, Australia for the fruitful discussion on distributed estimation and acknowledge Yuting Wei from the Department of Statistics, UC Berkeley for her sincere help on the consistency proof.
