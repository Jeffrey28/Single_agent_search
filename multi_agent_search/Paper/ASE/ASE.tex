...
\title{\LARGE \bf
Distributed Bayesian Filters for Multirobot Network Using Latest-In-and-Full-Out Exchange Protocol of Observations}
...
The proposed scheme significantly reduces transmission burden and only rely on local communication, therefore suitable for applications in realistic situations.
...
\begin{IEEEkeywords}
Distributed filtering, multiagent system, consistency of estimation, consensus.
\end{IEEEkeywords}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{INTRODUCTION}
\IEEEPARstart{D}{istributed} filtering that focuses on using a group of networked robots to collectively infer environment status has been used for various applications, such as intruder detection \cite{chamberland2007wireless}, pedestrian tracking \cite{wang2007wlan} and \textcolor{blue}{underwater environmental monitoring \cite{pandey2015dynamic}}. 
...
\textcolor{blue}{One pioneering work was done by Rosencrantz et al. (2003), who proposed a selective communication scheme that enabled individual robots to communicate the most informative observation to other entities.
...
Ribeiro et al. (2006) \textcolor{blue}{proposed a distributed estimation method based on exchanging} quantized observations along with error-variance limits considering pragmatic signal models \cite{ribeiro2006bandwidth}.
...
\textcolor{blue}{Note that if the target is static, such as in the application of indoor search for stationary objects \cite{kulich2014single}, Markov motion model is simplified to be}
\small\begin{equation*}
...
\begin{rem}
	...
	Therefore, besides storing individual PDF \textcolor{blue}{ with size $O(M^2)$}, only current-step CB is stored in robot memory and all historical CBs can be discarded, which means that the size of occupied memory is \textcolor{blue}{$O(N+M^2)$}. 
...
Readers interested in motion planning of robots for target localization can refer to \cite{tisdale2009autonomous,sujit2011self,7225186}. %,furukawa2006recursive
...
\begin{figure}%[thpb]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\label{fig:sta_sen_sta_tar_entropy}
	\end{subfigure}	
	~
	\begin{subfigure}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{figures/mov_sen_mov_tar_entropy}
		\label{fig:mov_sen_mov_tar_entropy}
	\end{subfigure}
\caption{Top figure: entropy of target PDF using static robots. Bottom figure: entropy of target PDF using moving robots.}
\end{figure}
