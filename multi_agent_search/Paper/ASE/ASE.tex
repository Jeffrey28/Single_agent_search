...
\begin{ntp}
Deploying swarms of robots for collecting environment information is receiving increasing interest in both military and civilian applications, such as surveillance, reconnaissance, search and rescue.
This work focuses on distributed estimation of the position of a hidden target in the environment.
One key challenge for effective estimation is limited communication range and resource of robots.
Traditional methods usually incur large transmission burden or assume that all robots can communicate with each other.
This paper presents a distributed estimation scheme, called distributed Bayesian filtering, that is based on local exchange of sensor measurements among neighboring robots.
The proposed scheme can significantly reduce transmission burden and only rely on communication among neighboring robots, which is suitable for realistic applications.
In addition, theoretical analysis ensures that all robots can correctly localize the true position of the target, when enough observations are obtained.
...
In this scenario, the six robots are controlled to move in circular trajectories periodically while the target moves linearly. Readers interested in motion planning of robots can refer to \textcolor{blue}{\cite{tisdale2009autonomous,sujit2011self,7225186}}. %,furukawa2006recursive
