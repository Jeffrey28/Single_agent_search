...
%% 2016 January. This code is based on main_data_exchange_switching and uses
% simulated data from P3dx/MobileSim
...
file_folder = '/Users/changliu/Documents/Git/Autonomous_agent_search/multi_agent_search/experiment/data';
file_name1 = 'r1_tar_pos_mobilesim_010416.mat';
load(fullfile(file_folder,file_name1),'full_obs','pose_rd');
r1_full_obs = full_obs;
r1_pose_rd = pose_rd;
file_name2 = 'r2_tar_pos_mobilesim_010416.mat';
load(fullfile(file_folder,file_name2),'full_obs','pose_rd');
r2_full_obs = full_obs;
r2_pose_rd = pose_rd;
...
for i=1:NumOfRobot
    if (i == 1)
        rbt(i).x = r1_pose_rd.pos(1,1);
        rbt(i).y = r1_pose_rd.pos(2,1);
    elseif (i == 2)
        rbt(i).x = r2_pose_rd.pos(1,1);
        rbt(i).y = r2_pose_rd.pos(2,1);
    end
    rbt(i).speedLimit = 0;
...
% binary sensor model
sigmaVal=0.25*eye(2); % covariance matrix for the sensor
...
rbt(1).top(1).neighbour=2;
rbt(2).top(1).neighbour=1;

rbt(1).top(2).neighbour=0;
rbt(2).top(2).neighbour=0;
...
while (1) %% Filtering Time Step
    if (count > size(r1_full_obs,1)) || (count > size(r2_full_obs,1))
        break
    end
    figure(1); clf(hf1);
    for i = 1:NumOfRobot
...
%     end
    ...
    for i=1:NumOfRobot % Robot Iteration
        if (i == 1)
            rbt(i).z = r1_full_obs(count,:);
        elseif (i == 2)
            rbt(i).z = r2_full_obs(count,:);
        end
    end
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Bayesian Updating
...
%                 end
                ...
                for i=1:NumOfRobot
                        ...
                        if (~isempty(rbtBuffer{i}.rbt(i).k)) && (rbtBuffer{i}.rbt(i).z(1) ~= -1) && (rbtBuffer{i}.rbt(i).z(2) ~= -1)
